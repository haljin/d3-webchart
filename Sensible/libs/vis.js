...
BubbleChart = (function() {
	...
	BubbleChart.prototype.load_data = function(start, end){
		...
		d3.json("http://localhost:8000/data/call_log/" + chart.token,  function(data) { 
			if(!runOnce[0])
			{
				runOnce[0] = true;
				chart.callData = data;
		...
		d3.json("http://localhost:8000/data/sms/" + chart.token,  function(data) { 
			if(!runOnce[1])
			{
				runOnce[1] = true;
				chart.smsData = data;
		...
		d3.json("http://localhost:8000/data/bluetooth/" + chart.token,  function(data) { 
			if(!runOnce[2])
			{
				runOnce[2] = true;
				chart.btData = data;
		...
		chart.show_loading_screen(chart);
	...
	BubbleChart.prototype.show_loading_screen = function(chart) {
		var w = 460,
		h = 200,
		...
		fields.previous = fields.value;
		if (fields.value != fields.size)
		{  
			fields.value += 1;
		}
		else
